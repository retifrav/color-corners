# original: https://github.com/OlivierLDff/QtStaticCMake/blob/master/QtStaticCMake.cmake

if(NOT QT_HOME)
    message(FATAL_ERROR "QT_HOME is not set, it should point to Qt installation path")
endif()
...
macro(qt_generate_qml_plugin_import TARGET)
    ...
    set(
        QT_STATIC_MULTI_VALUE_ARG EXTRA_PLUGIN
    )
    ...
    cmake_parse_arguments(
        ARGSTATIC
        "${QT_STATIC_OPTIONS}"
        "${QT_STATIC_ONE_VALUE_ARG}"
        "${QT_STATIC_MULTI_VALUE_ARG}"
        ${ARGN}
    )
    ...
    if(NOT QT_STATIC_QML_DIR)
        set(QT_STATIC_QML_DIR "${QT_HOME}/qml")
    ...
    if(QT_STATIC_QML_SRC)
        # Debug
        if(QT_STATIC_VERBOSE)
            message(STATUS "Get Qml Plugin dependencies for ${QT_STATIC_TARGET}. qmlimportscanner path is ${QT_HOME}/bin/qmlimportscanner. RootPath is ${QT_STATIC_QML_SRC} and importPath is ${QT_STATIC_QML_DIR}.")
        endif()

        # Get Qml Plugin dependencies
        execute_process(
            COMMAND ${QT_HOME}/bin/qmlimportscanner -rootPath ${QT_STATIC_QML_SRC} -importPath ${QT_STATIC_QML_DIR}
        ...
        foreach(JSON_OBJECT ${QT_STATIC_QML_IMPORT_JSON_OBJ})
            ...
            if(CMAKE_MATCH_1 AND NOT ${CMAKE_MATCH_1} STREQUAL "")
                ...
                if(_PLUGIN_INDEX EQUAL -1)
                    #message(STATUS "QML plugin: ${CMAKE_MATCH_1}")
        ...
        #message(STATUS "List of plugins: ${QT_STATIC_QML_DEPENDENCIES_PLUGINS}")

        # TODO add a proper blacklisting mechanism
        foreach(plgn ${QT_STATIC_QML_DEPENDENCIES_PLUGINS})
            #message(STATUS "plgn: ${plgn}")
            if(${plgn} MATCHES "SomeBlacklistedPlugin") #"WorkerScript"
                if(QT_STATIC_VERBOSE)
                    message(STATUS "${plgn} is blacklisted, removing from the list")
                endif()
                list(REMOVE_ITEM QT_STATIC_QML_DEPENDENCIES_PLUGINS ${plgn})
            endif()
        endforeach()

        #message(STATUS "List of plugins, filtered: ${QT_STATIC_QML_DEPENDENCIES_PLUGINS}")
        ...
        file(WRITE ${QT_STATIC_QML_PLUGIN_SRC_FILE} "// File Generated via CMake script during build time.\n"
            ...
            set(QT_STATIC_ALL_PLUGINS
                ${QT_STATIC_QML_DEPENDENCIES_PLUGINS}
                ${ARGSTATIC_EXTRA_PLUGIN}
            )
...
macro(qt_generate_plugin_import TARGET)
    ...
    file(WRITE ${QT_STATIC_PLUGIN_SRC_FILE} "// File Generated via CMake script during build time.\n"
        ...
        # Get all available Qt${QT_VERSION_MAJOR} module
        file(GLOB QT_STATIC_AVAILABLES_QT_DIRECTORIES
            LIST_DIRECTORIES true
            RELATIVE ${QT_HOME}/lib/cmake
            ${QT_HOME}/lib/cmake/Qt${QT_VERSION_MAJOR}*)
        foreach(DIR ${QT_STATIC_AVAILABLES_QT_DIRECTORIES})
            ...
            if(DIR_PRESENT)
                ...
                if(DIR_PLUGIN_CONTENT)
                    # TODO add a proper blacklisting mechanism
                    if (${DIR} MATCHES "SomeBlacklistedPlugin") #VirtualKeyboard
                        if(QT_STATIC_VERBOSE)
                            message(STATUS "${DIR} is blacklisted, skipping")
                        endif()
                    else()
                        file(APPEND ${QT_STATIC_PLUGIN_SRC_FILE} "\n// ${DIR}\n")
                        # Parse Plugin to append to the list only if unique
                        foreach(PLUGIN ${DIR_PLUGIN_CONTENT})
                            # List form is Qt${QT_VERSION_MAJOR}::NameOfPlugin, we just need NameOfPlugin
                            ...
                            if(PLUGIN_NAME)
                                ...
                                if(_PLUGIN_INDEX EQUAL -1)
                                    ...
                                    # TODO add a proper blacklisting mechanism
                                    if (${PLUGIN_NAME} MATCHES "SomeBlacklistedPlugin") #VirtualKeyboard
                                        if(QT_STATIC_VERBOSE)
                                            message(STATUS "${PLUGIN_NAME} is blacklisted, skipping")
                                        endif()
                                    else()
                                        list(APPEND QT_STATIC_DEPENDENCIES_PLUGINS ${PLUGIN_NAME})
                                        file(APPEND ${QT_STATIC_PLUGIN_SRC_FILE} "Q_IMPORT_PLUGIN(${PLUGIN_NAME});\n")

                                        set(PLUGIN_LIBRARY Qt${QT_VERSION_MAJOR}::${PLUGIN_NAME})
                                        if(QT_STATIC_VERBOSE)
